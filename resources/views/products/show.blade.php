            ...
            <div class="lg:grid lg:grid-cols-2 lg:gap-x-8">
                ...
                <div class="p-8 lg:p-12 flex flex-col justify-center">
                    ...
                    <div class="mt-10 flex flex-col sm:flex-row gap-4">
                        @if($product->quantity > 0)
                            <form action="{{ route('cart.add', ['slug' => $product->slug]) }}" method="POST" class="flex-1">
                                @csrf
                                <button type="submit" class="w-full bg-brand-600 border border-transparent rounded-full py-4 px-8 flex items-center justify-center text-base font-medium text-white hover:bg-brand-700 focus:outline-none shadow-lg shadow-brand-500/30 transition-all text-center">
                                    Thêm vào giỏ
                                </button>
                            </form>
                        @else
                            <button type="button" disabled class="flex-1 bg-gray-300 border border-transparent rounded-full py-4 px-8 flex items-center justify-center text-base font-medium text-gray-500 cursor-not-allowed text-center">
                                Hết Hàng
                            </button>
                        @endif
                        <a href="{{ route('contact') }}" class="flex-1 bg-gray-900 border border-transparent rounded-full py-4 px-8 flex items-center justify-center text-base font-medium text-white hover:bg-gray-800 focus:outline-none shadow-lg transition-all text-center">
                            Yêu Cầu Báo Giá
                        </a>
                    </div>
                </div>
